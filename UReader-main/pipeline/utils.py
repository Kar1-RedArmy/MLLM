...
def batchify(batch):
    ...
    patch_positions = [data["patch_positions"] if data["patch_positions"] is not None else None for data in batch]
    if all([img is None for img in image]):
        image = None
        patch_positions = None
    else:
        image = torch.cat([img for img in image if img is not None], dim=0)
        patch_positions = torch.cat([patch for patch in patch_positions if patch is not None], dim=0)
    ...
    non_padding_mask = non_padding_mask[:,:max_length-1]
    non_media_mask = non_media_mask[:,:max_length-1]
    prompt_mask = prompt_mask[:,:max_length-1]
    # ic(text.shape)
...
def add_config_args(config, args):
    from icecream import ic
