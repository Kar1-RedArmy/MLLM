...
class MplugOwlVisionEncoder(nn.Module):
    ...
    def __init__(self, config: MplugOwlVisionConfig):
        ...
        self.config = config
        args = get_args()
        if args.enable_vit_adapter:
            adapter_layer_indices = eval(args.adapter_layer_indices)
            self.global_layer = [True if i in adapter_layer_indices else False for i in range(self.num_layers)]
        else:
    ...
    def forward(
        ...
        for index, encoder_layer in enumerate(self.layers):
            ...
            if self.global_layer is not None and self.global_layer[index]:
                origin_mask = attention_mask
                num_token = hidden_states.shape[0]
                # seq B D
                assert cut_index is not None
                hidden_states = hidden_states.permute(1,0,2) # -> B seq D
                patch_group = [hidden_states[cut:] if ci==len(cut_index)-1 else hidden_states[cut:cut_index[ci+1]] for ci,cut in enumerate(cut_index)]
                patch_group = [einops.rearrange(_, 'num_patch num_token D -> (num_patch num_token) D') for _ in patch_group]
                raw_seq_len = [_.shape[0] for _ in patch_group]
                patch_mask = [torch.ones(_, dtype=torch.long, device=hidden_states.device) for _ in raw_seq_len]
                patch_mask = pad_sequence(patch_mask,batch_first=True,padding_value=0).bool() # 0 denotes element should be ignored in CoreAttention
                patch_group = pad_sequence(patch_group,batch_first=True,padding_value=0.) # -> B (num_patch num_token + pad) D

                hidden_states = patch_group.contiguous()
                attention_mask = patch_mask.contiguous() # [b, np, sq, sk]
            ...
            if self.global_layer is not None and self.global_layer[index]:
                ...
                # cat back to bs
                hidden_states = torch.cat([einops.rearrange(hidden_states[bi,:_],'(num_patch num_token) D -> num_patch num_token D', num_token=num_token) for bi,_ in enumerate(raw_seq_len)],dim=0).contiguous()
                # should be torch.Size([257, 9, 1024])
            if output_attentions:
...
class MplugOwlVisualAbstractorModel(MplugOwlPreTrainedModel):
    def __init__(self, config: MplugOwlVisualAbstractorConfig, language_hidden_size):
        ...
        args = get_args()
        self.cut_num = 15 # TODO hard code
        if args.enable_vit_cut_embedding:
            self.enable_vit_cut_embedding=True
            # 如果vit启动了 那v2t那就不要用了
        else:
            self.enable_vit_cut_embedding=False
    ...
    def forward(
        ...
        if self.patch_pos_embed_type == 'pre':
            ...
            encoder_hidden_states = pad_sequence(patch_group,batch_first=True,padding_value=0.) # -> B (num_patch num_token + pad) D
        ...
        if self.patch_pos_embed_type == 'post':
            if self.enable_vit_cut_embedding:
